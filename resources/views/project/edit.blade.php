                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{route('project.update',$project->id)}}" method="POST"
                            ...
                            @csrf
                            @method('PUT')
                            <x-input name="title" value="{{$project->title}}" />
                            <x-input_wrapper>
                            ...
                            <x-input_wrapper>
                                ...
                                <input type="text" name="dead_line" id="deadline" class="form-control"
                                    value="{{old('dead_line',$project->dead_line)}}">
                            ...
                            <div class="preview-img" id="preview_images">
                                @foreach ($project->images as $image)
                                <img src="{{asset('/storage/project/'.$image)}}" alt="profie image" class="thumbnail"
                                    id="profile">
                                @endforeach
                            </div>
                            <x-input_wrapper class="profile">
                                <x-label name="file (only pdf)" />
                                <input type="file" name="files[]" class="form-control" autocomplete="off" multiple
                                    accept="application/pdf">
                            </x-input_wrapper>
                            <div class="preview-file" mb-4>
                                @foreach ($project->files as $file)
                                <span>
                                    <a href="{{asset('/storage/project/'.$file)}}" target="_blank"><i
                            ...
                            <x-input_wrapper>
                                ...
                                <select name="leaders[]" class="form-control select-leader" multiple>
                                    ...
                                    <option value="{{$employee->id}}" @if (in_array($employee->name,
                                        $project->leaders()->pluck('name')->toArray()))
                                        selected
                                        @endif>{{$employee->name}}
                            ...
                            <x-input_wrapper>
                                ...
                                <select name="members[]" class="form-control select-member" multiple>
                                    ...
                                    @foreach ($employees as $employee)
                                    <option value="{{$employee->id}}" @if(in_array($employee->name,
                                        $project->members()->pluck('name')->toArray()))
                                        selected
                                        @endif>{{$employee->name}}
                                        ({{$employee->employee_id}})
                                    </option>
                            ...
                            <x-input_wrapper>
                                ...
                                <select name="priority_id" class="form-control select-priority">
                                    ...
                                    <option value="{{$priority->id}}" {{$priority->id
                                        == old('priority_id',$project->priority_id) ? 'selected' :
                                        '-'}}>{{$priority->name}}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <x-input_wrapper>
                                ...
                                <select name="status_id" class="form-control select-status">
                                    ...
                                    <option value="{{$status->id}}" {{$status->id
                                        == old('status_id',$project->status_id) ? 'selected' : '-'}}>{{$status->name}}
                                    </option>
                                    @endforeach
                                </select>
